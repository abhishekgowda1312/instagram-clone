...
function App() {
  ...
  // use react hook for posts and their state
  ...
  useEffect(() => {
    ...
    //everytime a post is added, the code is fired
  }, [])
  //[] -> will run once, when the page loads
  //[posts] - > will run once when the page loads but also each time when the posts change state
  const signUp = (event) => {
